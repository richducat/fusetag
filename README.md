...
For a production-like preview, build then serve the static output:
```bash
npm run preview
```
...
This repo ships with a GitHub Actions workflow (`.github/workflows/gh-pages.yml`) that builds the
static Next.js export and publishes it to GitHub Pages on each push to `main` or when manually
triggered.
